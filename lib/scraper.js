  ...
  home: async () => {
    try {
      ...
      if (latestRaw.length === 0 && trendingRaw.length === 0 && forYouRaw.length === 0) {
        ...
        trendingRaw = cachedLatest.slice(0, 50);
        // forYou ambil 100 berikutnya
        forYouRaw = cachedLatest.slice(50, 150);
      }

      const latest = latestRaw.map(b => mapBook(b));
      const trending = trendingRaw.slice(0, 20).map((b, i) => ({
        ...mapBook(b, `#${i+1}`),
        rank: `#${i+1}`
      }));
      ...
      return {
        ...
        trending: trending.length > 0 ? trending : latest.slice(0, 10),
        foryou: forYouRaw.map(b => mapBook(b)),
        source: `dramabox.com/in - Official API + Cached ${cachedLatest.length} dramas`
      };
    } catch (e) {
